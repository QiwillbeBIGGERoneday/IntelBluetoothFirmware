...
void IntelBluetoothFirmware::publishReg(bool isSucceed, const char *fwName)
{
    ...
    // Also patch the legacy AppleUSBDevice representation, which bluetoothd reads.
    OSDictionary *matching = IOService::serviceMatching("AppleUSBDevice");
    if (matching != nullptr)
    {
        ...
        if (iter != nullptr)
        {
            ...
            while ((svc = OSDynamicCast(IOService, iter->getNextObject())) != nullptr)
            {
                OSNumber *vid = OSDynamicCast(OSNumber, svc->getProperty("idVendor"));
                OSNumber *pid = OSDynamicCast(OSNumber, svc->getProperty("idProduct"));
                if (vid != nullptr && pid != nullptr &&
                    vid->unsigned32BitValue() == 0x8087 &&
                    pid->unsigned32BitValue() == 0x29)
                {
                    svc->setProperty("USB Product Name", "Bluetooth USB Host Controller");
                    svc->setProperty("USB Vendor Name", "Intel Corporation");
                }
            }
            iter->release();
        }
